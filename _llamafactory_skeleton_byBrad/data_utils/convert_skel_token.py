import re
from llamafactory.extras.constants import (SKELETON_TOKEN_BASE, SKELETON_FRAME_BREAK, 
...
def get_skeleton_token_str_wTextualBodyPart_SplitByFrame(skeleton_indices, task='Vid2Skel', task_context=None):
    frame_strings = []
    for frame_id, frame_indices in enumerate(skeleton_indices): # 遍历每一帧
        ...
        # 将一帧内所有部位的字符串连接起来
        if task_context == 'history':
            frame_word = 'History frame'
        elif task_context == 'future':
            frame_word = 'Future frame'
        else:
            frame_word = 'Frame'
        frame_string = f"{frame_word} {frame_id + 1}: " + "".join(part_strings)
    ...
    if task == 'SkelPred':
        if task_context == 'history':
            ...
            answer_prefix = f"There are {len(skeleton_indices)} future frames in total. Here are the skeleton tokens for {len(BODY_PART_TOKENS)} body parts in each frame:\n"
        else:
            raise ValueError
    else:
